...
class TestAscendAttentionBackendImpl310(TestBase):
    ...
    def test_mtp_verification_uses_explicit_causal_mask(
        self,
        ...
        metadata.query_start_loc = torch.tensor([0, 2], dtype=torch.int32)
        metadata.seq_lens = torch.tensor([34], dtype=torch.int32)
        metadata.block_tables = torch.zeros(1, 5, dtype=torch.int32)
        metadata.query_lens_cpu = torch.tensor([2], dtype=torch.int32)
        ...
        mock_get_splitfuse_mask.return_value = explicit_mask
